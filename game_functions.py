...
def check_events(screen,settings,chessmans,chessboard):
    ...
    for event in pygame.event.get():
        if event.type in (pygame.QUIT, pygame.KEYDOWN):
            sys.exit()
        elif settings.user_round and event.type == pygame.MOUSEBUTTONDOWN:
...
def put_chessman(screen,color,position,settings,chessmans,chessboard):
    ...
    real_pos,draw_pos = cal_realpos(settings, position, chessboard)
    print(real_pos)
    if settings.user_round and real_pos[0]!=None and real_pos[1]!=None and can_put(chessboard,real_pos):
...
def cal_realpos(settings,position,chessboard):
    ...
    for i in chessboard.pos_map['screen_pos']:
        ...
        if (position[1] < (i + int(chessboard.gap/2)) ) and (position[1] >(i - int(chessboard.gap/2))):
            ...
            draw_y = i
        index += 1
    return [real_x,real_y],[draw_x,draw_y]